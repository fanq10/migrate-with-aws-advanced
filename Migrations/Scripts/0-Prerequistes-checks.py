...
def check_windows(Servers_Windows, CEServerIP):
    windows_results = []
...
def check_tcp_connectivity(ssh, host, port, s_result):
    ...
    if ssh is not None:
        ...
        try:
            ...
            ssh_err = "Exception while running the command " + cmd + \
                      " due to " + str(e)
    else:
        ssh_err = 'Unable to connect! SSH is null'
...
def main(arguments):
    ...
    if len(Servers_Windows) > 0:
        ...
        print("*********************************************")
        print("")
        windows_results = check_windows(Servers_Windows, args.CEServerIP)
