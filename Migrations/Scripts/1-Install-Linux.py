...
def execute_cmd(host, username, key, cmd, using_key):
    ...
    try:
        ...
        error = "Unable to run the command " + cmd + " due to " + \
                str(io_error)
        print(error)
...
def install_wget(host, username, key_pwd, using_key):
    ...
    try:
        ...
        if not error:
            ...
            # Run the wget command and check if it got configured correctly
...
def install_cloud_endure(host, username, key_pwd, using_key, install_token):
    print("")
    print("")
    print("--------------------------------------------------------")
    ...
    try:
        ...
        if not check_python(host, username, key_pwd, using_key):
            if not check_python3(host, username, key_pwd, using_key):
                ...
                python_str = "python3"
        # Step 2 - run the linux installer
